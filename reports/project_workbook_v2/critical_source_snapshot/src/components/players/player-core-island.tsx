...
export async function PlayerCoreIsland({
  ...
  const teamKey = seasonStats?.teamId ?? identityTeamKey ?? undefined;
  ...
  const rowTeamMark = (teamId: string, rowSeason: string) => {
    if (useHistoricalBranding) {
      const era = resolveHistoricalTeamBrand(teamId, rowSeason, "era");
      if (era) return <HistoricalTeamMark brand={era} size="2xs" />;
    }
    return <TeamLogo teamKey={teamId} size="2xs" />;
  };
  ...
  const rowTeamLabel = (row: PlayerSeason) => {
    ...
    return resolveTeamBrand(row.teamId)?.abbr ?? row.teamName;
  };
  ...
  const rowStripeColor = (teamId: string, rowSeason: string) => {
    ...
    return resolveTeamBrand(teamId)?.primary ?? "var(--primary)";
  };
