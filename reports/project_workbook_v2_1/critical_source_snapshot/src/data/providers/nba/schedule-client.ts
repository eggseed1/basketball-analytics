...
function canonicalNbaTeamId(nbaTeamId: string): string {
  return (
    getCanonicalTeamFromProvider("nba", nbaTeamId)?.canonicalTeamId ??
    nbaTeamId
  );
}
...
function parseScheduleGame(
  ...
  if (!id) return null;
  const homeProviderId = String(raw.homeTeam?.teamId ?? "");
  const awayProviderId = String(raw.awayTeam?.teamId ?? "");
  if (!homeProviderId || !awayProviderId) return null;
  const homeId = canonicalNbaTeamId(homeProviderId);
  const awayId = canonicalNbaTeamId(awayProviderId);
  ...
  const leaders: ScheduleLeader[] = (raw.pointsLeaders ?? [])
    ...
    .map((l) => ({
      ...
      teamId: canonicalNbaTeamId(String(l.teamId ?? "")),
      points: l.points,
    }));
  ...
  return {
    game: {
      ...
      homeTeamName: nbaTeamName(homeProviderId, homeAbbr ?? ""),
      awayTeamName: nbaTeamName(awayProviderId, awayAbbr ?? ""),
      homeScore: Number(raw.homeTeam?.score ?? 0),
      awayScore: Number(raw.awayTeam?.score ?? 0),
      gameType,
      status,
      teamIdProvider: "nba",
      homeProviderTeamId: homeProviderId,
      awayProviderTeamId: awayProviderId,
    },
