...
export default async function PlayerPage({
  ...
  const headlineExplain = headlineMetric
    ? explainMetric(
        headlineMetric.id === "darko"
          ? "darko"
          : headlineMetric.id === "ts"
            ? "trueShooting"
            : headlineMetric.id === "usg"
              ? "usage"
              : headlineMetric.id === "lebron"
                ? "lebron"
                : ""
      )
    : null;
  const headlineContext = headlineMetric
    ? buildStatContext({
        ...
        timeframe: season,
        learnHref: headlineExplain?.learnHref,
      ...
      <div className="grid items-start gap-4 lg:grid-cols-12">
        ...
        <div className="flex flex-col gap-4 lg:col-span-8">
          ...
          <p className="text-[13px] text-muted-foreground">
            ...
            {" · "}
            pick a second name on the compare page.
          </p>
        </div>
      </div>
      ...
      >
        <div className="flex flex-wrap items-end justify-between gap-2">
          <div>
            ...
            <p className="text-[13px] text-muted-foreground">
              In-depth stats by season · row accent = team that season.
            </p>
          </div>
        </div>
        {career.length === 0 ? (
      ...
      <TeamWashCard teamKey={teamKey} className="flex flex-col gap-3 p-4 sm:p-5">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="text-[13px] text-muted-foreground">
              In-depth stats for each game · {season}
            </p>
          </div>
        ...
        {seasonStats ? (
          <dl className="grid grid-cols-2 gap-3 rounded-xl border border-border bg-white/50 p-3 sm:grid-cols-4 lg:grid-cols-6">
            <MiniStat label="GP" value={formatNumber(seasonStats.gamesPlayed)} />
            <MiniStat label="MIN" value={formatMinutes(seasonStats.minutes)} />
            <MiniStat label="PTS" value={formatNumber(seasonStats.points)} />
            <MiniStat label="AST" value={formatNumber(seasonStats.assists)} />
            <MiniStat label="REB" value={formatNumber(seasonStats.rebounds)} />
            <MiniStat label="TS%" value={formatPct(seasonStats.trueShootingPct)} />
            <MiniStat label="USG" value={formatPct(seasonStats.usagePct)} />
              ...
              <tbody className="divide-y divide-border">
                {gameLog.map((g) => (
                  <tr key={g.id} className="hover:bg-secondary/40">
                    ...
                    <td className="px-2 py-2 text-right tabular-nums">{g.points}</td>
