 ...
 * Live roster synthesis has identical semantics in local and production; the
 * committed snapshot remains the deterministic fallback when a refresh fails.
 */
...
const ROOT = () => path.join(process.cwd(), "data", "front-office", "v1");
...
export function resolveFrontOfficeFranchiseId(teamKey: string): string | null {
...
export function loadFrontOfficeManifest() {
  return readJson<{
    snapshotDate: string; retrievedAt: string; sourceHash: string; status: string;
    season: string; teamsWithPayroll: number; playersWithSalary: number;
    capabilities: FrontOfficeLeagueSnapshot["capabilities"];
  }>(path.join(ROOT(), "manifest.json"));
}

export function loadTeamFrontOfficeSlice(franchiseId: string): TeamSlice | null {
  const id = resolveFrontOfficeFranchiseId(franchiseId);
  if (!id) return null;
  return readJson<TeamSlice>(path.join(ROOT(), "teams", `${id}.json`));
}

export function buildTeamPayrollPresentation(slice: TeamSlice): TeamPayrollPresentation {
  const disclosures = [
  ...
  if (slice.cap.status === "PROJECTED") disclosures.unshift("Cap thresholds marked Projected are not official.");
  return {
    team: { franchiseId: slice.team.franchiseId, abbr: slice.team.abbr, displayName: slice.team.displayName },
    season: slice.meta.season,
    updatedAt: slice.meta.retrievedAt,
    snapshotStatus: slice.meta.status,
    capContext: slice.cap,
    summary: {
      playerSalaryCommitments: slice.team.payroll.playersWithSalary > 0 ? slice.team.payroll.playerSalaryCommitments : null,
...
export function buildTeamDraftAssetsPresentation(slice: TeamSlice): TeamDraftAssetsPresentation {
  ...
  const firsts = assets.filter((a) => a.round === 1 && a.ownershipStatus === "CURRENTLY_OWNED").length;
  const seconds = assets.filter((a) => a.round === 2 && a.ownershipStatus === "CURRENTLY_OWNED").length;
  return {
    franchise: { franchiseId: slice.team.franchiseId, abbr: slice.team.abbr, displayName: slice.team.displayName },
    updatedAt: slice.meta.retrievedAt,
    snapshotStatus: slice.meta.status,
    summary: {
      futureFirstsControlled: hasAssets ? firsts : null,
      futureSecondsControlled: hasAssets ? seconds : null,
      unavailableReason: hasAssets ? null : (slice.team.draftAssets.unavailableReason ?? "Draft asset data unavailable"),
    },
    assetsByYear,
    ...
    disclosures: hasAssets
      ? ["Own first and second round picks shown for 2027–2030.", "Traded, conveyed, protected, and swap-affected picks are not modeled yet."]
      : ["Draft asset ledger requires an authoritative current snapshot or validated starting ownership plus complete transaction chain.", "Showing unavailable state — never a false zero."],
  };
}

export function buildTeamFrontOfficeSummary(slice: TeamSlice): TeamFrontOfficeSummary {
  const id = slice.team.franchiseId;
  const salaryOk = slice.team.payroll.playersWithSalary > 0;
  const assets = slice.team.draftAssets.assets ?? [];
  const hasAssets = assets.length > 0;
  const firsts = hasAssets ? assets.filter((a) => a.round === 1 && a.ownershipStatus === "CURRENTLY_OWNED").length : null;
  const seconds = hasAssets ? assets.filter((a) => a.round === 2 && a.ownershipStatus === "CURRENTLY_OWNED").length : null;
  return {
    franchiseId: id,
    season: slice.meta.season,
    updatedAt: slice.meta.retrievedAt,
    playerSalaryCommitments: salaryOk ? slice.team.payroll.playerSalaryCommitments : null,
    ...
    capabilities: slice.capabilities,
    disclosures: hasAssets
      ? ["Draft picks are baseline own picks; trades and protections not modeled.", salaryOk ? "Salary figure is Player Salary Commitments (approximate cap space on Cap & assets)." : "Salary data unavailable for this franchise snapshot."]
      : ["Draft asset counts omitted — source unavailable.", salaryOk ? "Salary figure is Player Salary Commitments (approximate cap space on Cap & assets)." : "Salary data unavailable for this franchise snapshot."],
  };
}
...
function liveFrontOfficeEnabled(): boolean {
  return process.env.DISABLE_LIVE_FRONT_OFFICE !== "1";
}

export async function resolveTeamFrontOfficeSlice(franchiseId: string, season?: string | null): Promise<TeamSlice | null> {
  const id = resolveFrontOfficeFranchiseId(franchiseId);
  if (!id) return null;
  ...
  if (targetSeason === nowSeason) {
    if (liveFrontOfficeEnabled()) {
      try {
        const { buildLiveTeamFrontOfficeSlice } = await import("@/data/front-office/build-live-front-office");
        const live = await buildLiveTeamFrontOfficeSlice(id);
        if (live) return live;
      } catch (error) {
        console.error("[front-office] live roster refresh failed", { franchiseId: id, error: error instanceof Error ? error.message : String(error) });
      }
    }
    return cached;
  }
