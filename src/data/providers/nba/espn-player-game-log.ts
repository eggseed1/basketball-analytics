...
function statReader(
  ...
  return {
    number: (...keys: string[]): number | null => numeric(raw(...keys)),
    pair: (...keys: string[]): [number | null, number | null] =>
      parsePair(raw(...keys)),
  };
}
...
function transformEvent(options: {
  ...
  return {
    ...
    plusMinus,
    ...(tsDenominator > 0 ? { trueShootingPct: points / tsDenominator } : {}),
    ...(fga > 0
      ? { effectiveFieldGoalPct: (fgm + 0.5 * tpm) / fga }
      : {}),
  };
}
...
export async function fetchCompleteEspnPlayerGameLog(
  ...
  for (const row of rows) {
    byGame.set(`${row.seasonType ?? "regular"}:${row.gameId}`, row);
  }
