...
export class NBADataProvider implements BasketballDataProvider {
  ...
  private playoffCareerCache = new TtlPromiseCache<PlayerSeason[]>();
  ...
  async getPlayerPlayoffCareerSeasons(
    playerId: string
  ): Promise<PlayerSeason[]> {
    return this.playoffCareerCache.getOrSet(
      playerId,
      CACHE_TTL_MS.career,
      () => this.fetchPlayerPlayoffCareer(playerId)
    );
  }
  ...
  private async fetchPlayerPlayoffCareer(
    playerId: string
  ): Promise<PlayerSeason[]> {
    try {
      ...
      const set = getResultSet(response, "SeasonTotalsPostSeason");
      if (!set) return [];
      const rows = resultSetToObjects(set);
      const info = await this.loadCommonPlayerInfo(playerId);
      const fromRow = rows[0] ? String(rows[0].PLAYER_NAME ?? "").trim() : "";
      ...
      for (const row of resultSetToObjects(set)) {
        ...
        if (!existing) {
          byId.set(gameId, {
            ...
            homeTeamName: isHome
              ? nbaTeamName(providerTeamId, abbr)
              : undefined,
            awayTeamName: isHome
              ? undefined
              : nbaTeamName(providerTeamId, abbr),
            ...
            status: "final",
            teamIdProvider: "nba",
            homeProviderTeamId: isHome ? providerTeamId : undefined,
            awayProviderTeamId: isHome ? undefined : providerTeamId,
          });
        } else {
          if (isHome) {
            existing.homeTeamId = teamId;
            existing.homeTeamAbbr = abbr;
            existing.homeTeamName = nbaTeamName(providerTeamId, abbr);
            existing.homeScore = score;
            existing.homeProviderTeamId = providerTeamId;
          } else {
            existing.awayTeamId = teamId;
            existing.awayTeamAbbr = abbr;
            existing.awayTeamName = nbaTeamName(providerTeamId, abbr);
            existing.awayScore = score;
            existing.awayProviderTeamId = providerTeamId;
          }
        }
      }
    }
  ...
  private async fetchGameBoxScore(
    ...
    if (/^40\d{7,}$/.test(gameId)) {
      ...
      const { transformEspnBoxScore } = await import(
        "@/data/transformers/espn"
      );
      const { canonicalSeasonFromStartYear } = await import(
        "@/data/providers/historical/season-range"
      );
      ...
      try {
        const summary = await espnFetchJson<{
          ...
          boxscore?: { players?: unknown[] };
          gameInfo?: { date?: string };
        }>(url, { ttlMs: CACHE_TTL_MS.boxScore, retries: 2 });
        ...
        const transformed = transformEspnBoxScore(
          summary as Parameters<typeof transformEspnBoxScore>[0],
          season
        );
        if (!transformed?.game?.id) return null;
        return { game: transformed.game, players: transformed.players };
      } catch {
        return null;
      }
    }
    ...
    if (teamRows.length >= 2) {
      ...
      if (!homeTeamId) {
        ...
        for (const row of teamRows) {
          ...
          const canonical =
            getCanonicalTeamFromProvider("nba", id)?.canonicalTeamId ?? id;
          if (canonical === homeTeamId || id === homeProviderTeamId) {
            homeScore = n(row, "PTS");
            homeProviderTeamId = homeProviderTeamId || id;
          }
          if (canonical === awayTeamId || id === awayProviderTeamId) {
            awayScore = n(row, "PTS");
            awayProviderTeamId = awayProviderTeamId || id;
          }
        }
      }
    } else if (playerSet) {
    ...
    const game: Game = {
      ...
      homeTeamName:
        known?.homeTeamName ??
        nbaTeamName(homeProviderTeamId || homeTeamId),
      awayTeamName:
        known?.awayTeamName ??
        nbaTeamName(awayProviderTeamId || awayTeamId),
      homeScore,
      awayScore,
      gameType: known?.gameType ?? "regular",
      status: "final",
      teamIdProvider: "nba",
      homeProviderTeamId: homeProviderTeamId || undefined,
